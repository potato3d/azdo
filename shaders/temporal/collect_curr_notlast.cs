...
layout(std430, binding = 10) buffer curr_visible_in
{
    readonly uint data[];
} input_visible_curr;

layout(std430, binding = 11) buffer last_visible_in
{
    readonly uint data[];
} input_visible_last;
...
void main()
{
    ...
    if(input_visible_curr.data[instance_id] == 0 || input_visible_last.data[instance_id] == 1)
    {
        return;
    }
